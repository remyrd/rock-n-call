...
(defn pto-xls-selection
  "Selects xlsx file containing PTO information"
  [{:keys [fx/context]}]
  (let [ptos (fx/sub-val context :ptos)]
    {:fx/type  :h-box
     :spacing  10
     :children [{:fx/type :label
                 :text    "Choose a PTO Excel Sheet: "}
                {:fx/type   :button
                 :text      "Choose"
                 :on-action {:event-type ::e/choose-pto-file}}
                {:fx/type :label
                 :text    (if ptos
                            (str (:status (first ptos)))
                            "Nothing loaded yet...")}]}))
